...
def load_and_train():
    df = pd.read_csv(
        "https://raw.githubusercontent.com/hazmaali9708/Car-Price-Prediction-/main/data/CarPrice_Assignment.csv"
    )
...
st.markdown("Enter car details and get price prediction")
st.divider()

col1, col2, col3 = st.columns(3)

with col1:
    brands = ['alfa-romeo','audi','bmw','chevrolet','dodge','honda','isuzu',
...
with col2:
    ...
    doornumber = st.selectbox("Doors", [2,4])

with col3:
    carlength = st.slider("Length", 140, 210, 175)
    carwidth = st.slider("Width", 60, 75, 67)
    carheight = st.slider("Height", 45, 60, 54)
    curbweight = st.slider("Weight", 1500, 4500, 2500)

st.divider()

enginetype = st.selectbox("Engine Type", ['ohc','ohcv','ohcf','dohc','dohcv','l','rotor'])
cylindernumber = st.selectbox("Cylinders", [2,3,4,5,6,8,12])
enginesize = st.slider("Engine Size", 60, 330, 130)
...
compressionratio = st.slider("Compression", 7.0, 23.0, 9.0)

citympg = st.slider("City MPG", 13, 50, 25)
highwaympg = st.slider("Highway MPG", 16, 55, 30)
peakrpm = st.slider("RPM", 4150, 6600, 5200)
wheelbase = st.slider("Wheelbase", 86, 121, 98)

if st.button("Predict Price"):

    luxury_set = ['bmw','mercedes-benz','jaguar','porsche','buick','volvo']
    ...
    is_luxury = 1 if brand in luxury_set else 0
